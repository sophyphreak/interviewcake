function getProductsOfAllIntsExceptAtIndex(intArray) {
  if (intArray.length < 2) {
    throw 'input array needs at least two values';
  }

  let currentMultiplier = 1;
  let resultArray = intArray.map(() => 1);

  for (let index = 1; index < intArray.length; index++) {
    currentMultiplier *= intArray[index - 1];
    resultArray[index] *= currentMultiplier;
  }
  currentMultiplier = 1;
  for (let index = intArray.length - 2; index >= 0; index--) {
    currentMultiplier *= intArray[index + 1];
    resultArray[index] *= currentMultiplier;
  }
